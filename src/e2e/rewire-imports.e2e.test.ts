...
describe('CLI: generated import rewiring (e2e)', () => {
  ...
  test('rewires the vendored graph into relative paths that Node can resolve', async () => {
    ...
    expect(alpha).toContain("import { beta } from '../../beta/index.js';");
    expect(alpha).toContain("import { util } from '../../gamma/util.js';");
    ...
    expect(await readModule('beta', 'index.js')).toContain(
      "import { gamma } from '../gamma/index.js';",
    );

    const run = await runAlphaOnNode();
    expect(run.stderr).toBe('');
    expect(run.exitCode).toBe(0);
    expect(run.stdout.trim()).toBe('beta+gamma|util|gamma');
  });
  ...
  test('verify accepts a rewired tree and rejects a hand-edited specifier', async () => {
    ...
    await writeFile(
      ...
      source.replace('../../beta/index.js', '../../gamma/index.js'),
      'utf8',
    );
  ...
  test('diff never shows the rewiring', async () => {
    ...
    expect(diff.stdout).not.toContain('../../beta/index.js');
  });
  ...
  test('patch captures only the real edit, in the patched tree\'s own specifiers', async () => {
    ...
    expect(contents).not.toContain('../../beta/index.js');
    expect(contents).not.toContain('../../gamma/util.js');
  ...
  test('a captured patch replays through sync with rewiring reapplied', async () => {
    ...
    expect(rebuilt).toContain("import { beta } from '../../beta/index.js';");

    const run = await runAlphaOnNode();
    expect(run.exitCode).toBe(0);
    expect(run.stdout.trim()).toBe('beta+gamma|util|patched');
...
describe('CLI: import rewiring across an update (e2e)', () => {
  ...
  test('update finalize rebuilds the module with rewiring reapplied', async () => {
    ...
    expect(rebuilt).toContain("import { beta } from '../../beta/index.js';");
    expect(rebuilt).toContain("import { util } from '../../gamma/util.js';");
    expect(rebuilt).toContain("const label = 'patched';");
    ...
    expect(await readFile(join(cwd, 'inrepo_modules', 'alpha', 'src', 'extra.js'), 'utf8')).toBe(
      "export { gamma as extra } from '../../gamma/index.js';\n",
    );
