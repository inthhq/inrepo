...
export function parseLsRemote(raw: string): RemoteRefRow[] {
  const rows: RemoteRefRow[] = [];
  for (const line of raw.split('\n')) {
    const match = /^([0-9a-f]{40})\s+(\S+)$/i.exec(line.trim());
    if (!match) continue;
 ...
 */
export function pickRemoteCommit(rows: RemoteRefRow[], pattern: string): string | null {
 ...
 */
export async function resolveRemoteCommit(gitUrl: string, ref?: string | null): Promise<string> {
  const wanted = ref?.trim();
  if (wanted && FULL_SHA.test(wanted)) return wanted.toLowerCase();
