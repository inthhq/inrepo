...
import { parseLsRemote, pickRemoteCommit, resolveRemoteCommit } from './resolve-remote-commit.js';
...
describe('pickRemoteCommit', () => {
  ...
  test('matches HEAD by its advertised name', () => {
    expect(pickRemoteCommit([{ commit: SHA_A, ref: 'HEAD' }], 'HEAD')).toBe(SHA_A);
  });
...
describe('resolveRemoteCommit', () => {
  ...
  test('a full commit id resolves to itself without touching the remote', async () => {
    expect(await resolveRemoteCommit('this-url-does-not-exist', fx.c1.toUpperCase())).toBe(fx.c1);
  });

  test('explains that an abbreviated commit id has no moving tip', async () => {
    await expect(resolveRemoteCommit(fx.url, fx.c1.slice(0, 10))).rejects.toThrow(
      /looks like a commit id.*--ref/s,
    );
  });

  test('reports an unknown ref', async () => {
    await expect(resolveRemoteCommit(fx.url, 'no-such-branch')).rejects.toThrow(
      /Could not resolve ref "no-such-branch"/,
    );
  });
});
