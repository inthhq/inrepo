...
describe("diffTrees", () => {
  ...
  beforeEach(async () => {
    ...
    for (const root of [base, target]) {
      ...
      await writeFile(
        nodePath.join(root, "logo.bin"),
        new Uint8Array([0x89, 0x50, 0x00, 0x01])
      );
    }
    await writeFile(nodePath.join(base, ".cache-meta.json"), "{}\n", "utf-8");
  });
  ...
  test("renders hunks for a modified file", async () => {
    await writeFile(
      nodePath.join(target, "src", "index.ts"),
      "export const v = 99;\n",
      "utf-8"
    );
  ...
  test("renders additions and deletions", async () => {
    await writeFile(
      nodePath.join(target, "src", "local.ts"),
      "export const local = true;\n",
      "utf-8"
    );
    await rm(nodePath.join(target, "README.md"));
  ...
  test("renders mode changes and binary differences without literal blobs", async () => {
    await chmod(nodePath.join(target, "src", "index.ts"), 0o755);
    await writeFile(
      nodePath.join(target, "logo.bin"),
      new Uint8Array([0x89, 0x50, 0x00, 0x02])
    );
  ...
  test("ignores cache metadata and generated markers on either side", async () => {
    await writeFile(
      nodePath.join(target, ".inrepo-vendor.json"),
      '{"commit":"abc"}\n',
      "utf-8"
    );
    expect(await diffTrees({ baseRoot: base, targetRoot: target })).toBe("");
  ...
  test("--stat summarizes per file instead of showing hunks", async () => {
    ...
    expect(stat).toMatch(/2 files changed/u);
