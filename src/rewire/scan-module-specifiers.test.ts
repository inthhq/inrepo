...
function found(source: string): string[] {
  return scanModuleSpecifiers(source).map((entry) => `${entry.kind}:${entry.value}`);
}
...
describe('scanModuleSpecifiers', () => {
  ...
  test('leaves property calls named import or require alone', () => {
    const source = ['const a = obj.require("a");', 'const b = mod.import("b");'].join('\n');
