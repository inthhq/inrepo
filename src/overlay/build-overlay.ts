...
import type { CompareTreesResult } from "./compare-trees.js";
import { writeDeletionsFile } from "./deletions-file.js";
import { copyEntry, walkTree } from "./tree-utils.js";

const pathDepth = function pathDepth(relPosix: string): number {
  return relPosix.split("/").length;
};

const isWithinDir = function isWithinDir(
  relPosix: string,
  dirPosix: string
): boolean {
...
const shouldIgnoreRemovedPath = function shouldIgnoreRemovedPath(
  relPosix: string,
  typeChanges: Set<string>
): boolean {
  return [...typeChanges].some((changed) => relPosix.startsWith(`${changed}/`));
};

const collapseDeletions = async function collapseDeletions(
  pristineRoot: string,
  compare: CompareTreesResult
): Promise<string[]> {
  const pristineEntries = await walkTree(pristineRoot, {
  ...
  for (const relPosix of [...compare.removed].toSorted(
    (a, b) => pathDepth(a) - pathDepth(b)
  )) {
    if (shouldIgnoreRemovedPath(relPosix, typeChanges)) {
      continue;
    }
    if (
      collapsed.some(
        (entry) =>
          entry.endsWith("/") && isWithinDir(relPosix, entry.slice(0, -1))
      )
    ) {
      continue;
    }
    ...
    const subtree = [...pristineEntries.keys()].filter(
      (candidate) =>
        candidate === relPosix || candidate.startsWith(`${relPosix}/`)
    );
  ...
  return [...new Set(collapsed)].toSorted();
};

export const buildOverlay = async function buildOverlay(opts: {
  pristineRoot: string;
  moduleRoot: string;
  overlayRoot: string;
}): Promise<{ compare: CompareTreesResult; deletions: string[] }> {
  const compare = await compareTrees(opts.pristineRoot, opts.moduleRoot);
  const moduleEntries = await walkTree(opts.moduleRoot, {
    treatMissingAsEmpty: true,
  });
  const deletions = await collapseDeletions(opts.pristineRoot, compare);
  const stageParent = nodePath.dirname(opts.overlayRoot);
  await mkdir(stageParent, { recursive: true });
  const stageRoot = await mkdtemp(
    nodePath.join(stageParent, ".inrepo-overlay-")
  );
  ...
  try {
    ...
    await writeDeletionsFile(
      nodePath.join(stageRoot, ".inrepo-deletions"),
      deletions
    );

    const stagedEntries = await walkTree(stageRoot, {
      treatMissingAsEmpty: true,
    });
    await rm(opts.overlayRoot, { force: true, recursive: true });
