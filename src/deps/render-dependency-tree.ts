...
const shortCommit = function shortCommit(commit: string): string {
 ...
 */
export const renderDependencyTree = function renderDependencyTree(
  graph: DependencyGraph
): string {
  const byModule = new Map(
    graph.nodes.map((node) => [node.module, node] as const)
  );
  ...
  const walk = (
    ...
    for (const [index, [dependency, edge]] of children.entries()) {
      ...
      const notes: string[] = [];
      if (child.reused) {
        notes.push("already vendored");
      }
      if (path.has(child.module)) {
        notes.push("cycle");
      } else if (expanded.has(child.module)) {
        notes.push("deduped");
      }
      ...
      if (path.has(child.module) || expanded.has(child.module)) {
        continue;
      }
