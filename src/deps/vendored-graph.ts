 ...
 */
export const readVendoredGraph = async function readVendoredGraph(
  cwd: string
): Promise<LockGraph> {
...
export const graphRoots = function graphRoots(graph: LockGraph): string[] {
  return Object.keys(graph)
    .filter((name) => graph[name].root === true)
    .toSorted();
};

/** Map of dependency package name → vendored module id. */
export interface DependencyModuleMap {
 ...
 */
export const dependencyModules = function dependencyModules(
  graph: LockGraph,
  name: string
): DependencyModuleMap {
  const dependencies = graph[name]?.dependencies ?? {};
  const out: DependencyModuleMap = {};
 ...
 * dropping a package.
 */
export const orderByDependencies = function orderByDependencies<
  T extends { name: string; module?: string },
>(packages: T[], graph: LockGraph): T[] {
  const byName = new Map(
    packages.map((pkg) => [pkg.module ?? pkg.name, pkg] as const)
  );
  ...
  const visit = (name: string): void => {
    ...
    for (const dependency of Object.keys(
      ...
      if (edge && byName.has(edge.module)) {
        visit(edge.module);
      }
    }
    state.set(name, "done");
  ...
  for (const pkg of packages) {
    visit(pkg.module ?? pkg.name);
  }
  return out;
};

/** Every package in the graph reachable from `name`, including `name` itself. */
export const graphClosure = function graphClosure(
  graph: LockGraph,
  name: string
): string[] {
  ...
  while (queue.length > 0) {
    ...
    if (current == null || seen.has(current) || graph[current] == null) {
      continue;
    }
    seen.add(current);
