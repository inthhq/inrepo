...
import type { RemoteRefRow } from "../git/resolve-remote-commit.js";
...
const bareName = function bareName(name: string): string {
 ...
 */
export const versionTagCandidates = function versionTagCandidates(
  name: string,
  version: string
): string[] {
...
export const pickVersionTag = function pickVersionTag(
  rows: RemoteRefRow[],
  candidates: string[]
): VersionTag | null {
  ...
  for (const candidate of candidates) {
    ...
    if (peeled) {
      return { commit: peeled, ref: candidate };
    }
    const direct = byRef.get(`refs/tags/${candidate}`);
    if (direct) {
      return { commit: direct, ref: candidate };
    }
  }
  return null;
};
...
export const listRemoteTags = async function listRemoteTags(
  gitUrl: string
): Promise<RemoteRefRow[]> {
...
export const resolveVersionTag = async function resolveVersionTag(
  gitUrl: string,
  name: string,
  version: string
): Promise<VersionTag | null> {
  return pickVersionTag(
    await listRemoteTags(gitUrl),
    versionTagCandidates(name, version)
  );
};
