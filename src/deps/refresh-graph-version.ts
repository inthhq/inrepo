...
export interface GraphRangeViolation {
...
export interface RefreshGraphVersionInput {
  graph: LockGraph;
...
export interface RefreshGraphVersionResult {
 ...
 */
export const refreshGraphVersion = function refreshGraphVersion(
  input: RefreshGraphVersionInput
): RefreshGraphVersionResult {
  ...
  if (node.version !== version) {
    nodes[name] = { ...node, version };
  }

  const violations: GraphRangeViolation[] = [];
  for (const dependent of Object.keys(graph).toSorted()) {
    ...
    const next = { ...edges };
    let changed = false;
    for (const dependency of Object.keys(edges).toSorted()) {
      const edge = edges[dependency];
      if (edge.module !== name) {
        continue;
      }
      if (!satisfies(version, edge.range)) {
        violations.push({ dependency, dependent, range: edge.range });
      }
    ...
    if (changed) {
      nodes[dependent] = { ...current, dependencies: next };
    }
  }
