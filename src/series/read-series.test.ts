...
describe("readSeries", () => {
  ...
  test("returns an empty list when the series directory is missing", async () => {
    expect(await readSeries(nodePath.join(cwd, "nope"))).toEqual([]);
  });

  test("orders patches by filename and ignores non-patch files", async () => {
    await writeFile(nodePath.join(seriesDir, "0010-tenth.patch"), "", "utf-8");
    await writeFile(nodePath.join(seriesDir, "0002-second.patch"), "", "utf-8");
    await writeFile(nodePath.join(seriesDir, "0001-first.patch"), "", "utf-8");
    await writeFile(nodePath.join(seriesDir, "README.md"), "notes\n", "utf-8");
  ...
  test("rejects patch files that do not follow the numbering convention", async () => {
    await writeFile(nodePath.join(seriesDir, "fixup.patch"), "", "utf-8");
    await expect(readSeries(seriesDir)).rejects.toThrow(
      /expected NNNN-<slug>\.patch/u
    );
  });
  ...
  test("rejects duplicate patch numbers", async () => {
    await writeFile(nodePath.join(seriesDir, "0001-first.patch"), "", "utf-8");
    await writeFile(
      nodePath.join(seriesDir, "0001-also-first.patch"),
      "",
      "utf-8"
    );
    await expect(readSeries(seriesDir)).rejects.toThrow(
      /Duplicate patch number 0001/u
    );
  });
