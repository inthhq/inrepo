...
const PATCH_REF = { fileName: '0002-tighten-jsdoc-types.patch', path: '/tmp/x.patch', index: 2 };
...
describe('readSeriesHeaders', () => {
  ...
  test('returns the series in apply order', async () => {
    await writeFile(join(seriesDir, '0002-second.patch'), 'Subject: [PATCH] Second\n\n', 'utf8');
    await writeFile(join(seriesDir, '0001-first.patch'), 'Subject: [PATCH] First\n\n', 'utf8');

    const headers = await readSeriesHeaders(seriesDir);
    expect(headers.map((header) => header.subject)).toEqual(['First', 'Second']);
    expect(headers.map((header) => header.index)).toEqual([1, 2]);
    expect(headers[0].path).toBe(join(seriesDir, '0001-first.patch'));
  });

  test('returns an empty list when there is no series', async () => {
    expect(await readSeriesHeaders(join(cwd, 'nope'))).toEqual([]);
  });
});
