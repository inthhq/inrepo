...
Both install the same npm-published artifact. `inrepo` requires [Node.js](https://nodejs.org/) 20+.
...
- `inrepo_patches/<name>/` stores your team's committed changes, either as an ordered git patch series under `series/` or as legacy whole-file overlay snapshots.
- `inrepo_modules/<name>/` is generated output rebuilt from the lockfile plus those changes.
