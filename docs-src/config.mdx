...
Lockfile versions 1–3 remain readable. Version 4 is written when a module identity differs from its package source name; graph edges point to that exact module instance, and the module's lock entry retains its `source` name.
