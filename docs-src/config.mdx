...
For a monorepo package, filtering and patches are relative to `repositoryDirectory`, not the repository root. The selected directory is recorded in lockfile version 3 and replayed offline by `sync`, `verify`, `diff`, `patch`, and `update`. Use `inrepo add <name> --git <url> --repository-directory <path>` when npm metadata cannot provide it.
