...
The edges are recorded under `graph` in `inrepo.lock.json`. A versioned module instance raises the file to `lockfileVersion: 4`, separating its storage key from the lock entry's npm `source`. Versions 1–3 remain readable for projects without module instances. Every dependency entry pins an exact git URL and immutable commit, so `sync` and `verify` replay and check the whole graph offline with no registry access.
