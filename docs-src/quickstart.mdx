...
This vendors source without changing package.json dependency buckets.
...
Opt in when the generated local package should be wired into `dependencies` or `devDependencies`:

```bash
npx inrepo add <package> --dependency
npx inrepo add <package> -D
```

The choice is saved as `packageJson` on that package's inrepo config entry and replayed by `inrepo sync`.
