...
Resolution fails before anything is vendored when two packages need the same dependency at ranges no published version satisfies, when a dependency uses a source that cannot be pinned (`workspace:`, `file:`, `link:`, `catalog:`, `npm:` aliases, git or tarball URLs, dist-tags), or when a dependency has no usable `repository` URL or no tag for the resolved version. Each message names the dependency and the reason; the fix is to vendor that one package by hand with `npx inrepo add <dep> --git <url> --ref <ref>`. Overlapping ranges are not a conflict — `^1.0.0` and `>=1.2.0` unify onto the highest version satisfying both.
